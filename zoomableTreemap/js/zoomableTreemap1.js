...
d3.json("../json/TreeMapData1.json", function (err,data) {
  if (err) {
    throw err;
  }
...
function updateDrillDown(data) {
  if (svg) {
    ...
    svg = d3.select("#ZoomableTreemap").append("svg")
...
function TitleName(d) {
  return d.parent ? TitleName(d.parent) + " / " + d.data.shortName + " (" + formatNumber(d.value) + ")" : d.data.shortName + " (" + formatNumber(d.value) + ")";
}